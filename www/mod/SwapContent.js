  ...
  function SwapContent(cfg) {
        ...
        get: {
          ...
          itm: {
            int: function (v) {
              return (
                items[v] ||
                ERR("Value loc out of range", { v: v, items: items })
              );
            },
            str: function (v) {
              return (
                items[getLoc(v)] ||
                ERR("Item ref not recognized", { v: v, items: items })
              );
            },
            not: function (v) {
              return active || ERR("No item active", { items: items });
            },
          },
        },
        set: {
    ...
    function buildItems() {
      ...
      all.snc(itms, function (o) {
        ...
        if (ref && !is.num(ref) && !is.str(ref, "n") && !refs[ref]) {
          refs[ref] = num;
        }
    ...
    function getRef(v) {
      return (
        (!is(v) && cur) || (is.int(v) && keys[v]) || (is.str(v, 1) && loc) || ""
      );
    } //getRef()
    ...
    function setLoc(v) {
      ...
      if (!len) {
        return ERR('Can not set loc to "' + v + '", no items in SWAP.');
      }
      return (
        (is.int(v) && v > 0 && v <= len && setItem((loc = v))) ||
        ERR("Bad value or out of range", { v: v, len: len })
      );
    } //setLoc()
